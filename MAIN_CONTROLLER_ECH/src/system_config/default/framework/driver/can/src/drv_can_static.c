...
static CAN_TX_MSG_BUFFER  *drv_Message1;
static CAN_TX_MSG_BUFFER __attribute__((coherent, aligned(16))) can_message_buffer1[2*2*16];
...
void DRV_CAN1_Initialize(void)
{
    ...
    PLIB_CAN_BaudRatePrescaleSet(CAN_ID_2, 4); // set to 1 higher then ECAN tool


    /* Assign the buffer area to the CAN module.
      In this case assign enough memory for 2
      channels, each with 8 message buffers.*/
    PLIB_CAN_MemoryBufferAssign(CAN_ID_2, can_message_buffer1);

    /* Configure CAN_ID_2 Channel for CAN_TX_RTR_DISABLED operation. Allocate 32 message buffer, and assign low medium priority for transmissions. */
    PLIB_CAN_ChannelForTransmitSet(CAN_ID_2, CAN_CHANNEL0, 32, CAN_TX_RTR_DISABLED, CAN_LOW_MEDIUM_PRIORITY);
    /* Configure CAN_ID_2 Channel for CAN_RX_FULL_RECEIVE operation. Allocate 32 message buffer, and assign low medium priority for transmissions. */
    PLIB_CAN_ChannelForReceiveSet(CAN_ID_2, CAN_CHANNEL1, 32, CAN_RX_FULL_RECEIVE);
...
void DRV_CAN1_Deinitialize(void)
{
...
void DRV_CAN1_Open(void)
{
   /* Switch the CAN module ON */
    PLIB_CAN_Enable(CAN_ID_2);
}

void DRV_CAN1_Close(void)
{
   /* Switch the CAN module OFF */
    PLIB_CAN_Disable(CAN_ID_2);
}

bool DRV_CAN1_ChannelMessageTransmit(CAN_CHANNEL channelNum, int address, uint8_t DLC, uint8_t* message)
{

    int count = 0;
    if ( (PLIB_CAN_ChannelEventGet(CAN_ID_2, channelNum) & CAN_TX_CHANNEL_NOT_FULL) == CAN_TX_CHANNEL_NOT_FULL)
    {
        //drv_Message1 = (CAN_TX_MSG_BUFFER *) &can_message_buffer1[channelNum];
        drv_Message1 = PLIB_CAN_TransmitBufferGet(CAN_ID_2, channelNum);

        /* Check the address whether it falls under SID or EID,
         * SID max limit is 0x7FF, so anything beyond that is EID */
        if(address > 0x7FF)
        {
            drv_Message1->msgSID.sid     = (address>>18);
            drv_Message1->msgEID.eid     = (0x3FFFF)&(address);
...
bool DRV_CAN1_ChannelMessageReceive(CAN_CHANNEL channelNum, int address, uint8_t DLC, uint8_t* message)
{
    int EchoDLC = 0;
    CAN_RX_MSG_BUFFER *RxMessage;
    uint32_t tempAddress;
